

# 视频
* [win10家庭版安装Docker for Windows](https://www.cnblogs.com/samwu/p/10360943.html)
* [Windows10家庭版的功能中没有Hyper-V的解决方法](https://qq52o.me/2468.html)

# 有用的参考
* [在window下修改docker配置文件daemon.json](https://blog.csdn.net/supertor/article/details/83109759)
* [Windows 上的 Docker 引擎](https://docs.microsoft.com/zh-cn/virtualization/windowscontainers/manage-docker/configure-docker-daemon)
* [修改docker私库的地址（设置docker访问白名单）](https://blog.csdn.net/supertor/article/details/83143816)
* [Docker的配置文件 daemon.json 详解](https://blog.csdn.net/kozazyh/article/details/79795559)
